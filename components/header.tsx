// "use client";

// import { Button } from "@/components/ui/button";
// import {
//   Asset,
//   Avatar,
//   Edit,
//   Enhancer,
//   Home,
//   ImageIcon,
//   Menu,
//   Realtime,
//   Video,
// } from "@/hooks/use-icon";
// import { cn } from "@/lib/utils";
// import { Moon, Sun, ChevronRight, Bell, Headset, ImageUp } from "lucide-react";
// import { useTheme } from "next-themes";
// import Link from "next/link";
// import { usePathname } from "next/navigation";
// import { Sheet, SheetContent, SheetTitle, SheetTrigger } from "./ui/sheet";

// export function Header() {
//   const { theme, setTheme } = useTheme();
//   const pathname = usePathname();

//   const links = [
//     { href: "/", label: "Home", Icon: Home },
//     { href: "#", label: "Image", Icon: ImageIcon },
//     { href: "#", label: "Video", Icon: Video },
//     { href: "#", label: "Enhancer", Icon: Enhancer },
//     { href: "#", label: "Realtime", Icon: Realtime },
//     { href: "#", label: "Edit", Icon: Edit },
//     { href: "#", label: "Assets", Icon: Asset },
//   ];

//   return (
//     <header className="sticky top-0 z-50 w-full bg-background/80 backdrop-blur-sm">
//       <div className="flex h-16 items-center justify-between px-4">
//         {/* Mobile nav trigger */}
//         <Sheet>
//           <SheetTrigger asChild>
//             <Button
//               variant="ghost"
//               size="sm"
//               className="h-10 w-10 p-0 md:hidden"
//             >
//               <Menu />
//             </Button>
//           </SheetTrigger>
//           <SheetContent side="left" className="w-64">
//             <SheetTitle className="sr-only">Krea</SheetTitle>
//             <div className="flex flex-col gap-4 mt-6">
//               {links.map(({ href, Icon, label }, index) => (
//                 <Link
//                   key={href + index}
//                   href={href}
//                   className="flex items-center gap-3 p-2 rounded-md hover:bg-accent"
//                 >
//                   <Icon className="h-5 w-5" />
//                   <span>{label}</span>
//                 </Link>
//               ))}

//               {/* Extra links */}
//               <Link
//                 href="#"
//                 className="flex items-center gap-3 p-2 rounded-md hover:bg-accent"
//               >
//                 <ImageIcon />
//                 Gallery
//               </Link>
//               <Link
//                 href="#"
//                 className="flex items-center gap-3 p-2 rounded-md hover:bg-accent"
//               >
//                 <Headset className="h-5 w-5" />
//                 Support
//               </Link>
//               <Link
//                 href="#"
//                 className="flex items-center gap-3 p-2 rounded-md hover:bg-accent"
//               >
//                 <Bell className="h-5 w-5" />
//                 Notifications
//               </Link>
//             </div>
//           </SheetContent>
//         </Sheet>

//         {/* Desktop Logo */}
//         <div className="hidden md:flex items-center gap-4">
//           <div>
//             <svg
//               aria-label="Krea Logo"
//               width="24"
//               height="24"
//               viewBox="0 0 24 24"
//               fill="currentColor"
//               xmlns="http://www.w3.org/2000/svg"
//             >
//               <path d="M8.34 1.266c1.766-.124 3.324 1.105 3.551 2.802.216 1.612-.887 3.171-2.545 3.536-.415.092-.877.066-1.317.122a4.63 4.63 0 0 0-2.748 1.34l-.008.004-.01-.001-.006-.005-.003-.009q0-.009.005-.016a.04.04 0 0 0 .007-.022 438 438 0 0 1-.01-4.541c.003-1.68 1.33-3.086 3.085-3.21"></path>
//               <path d="M8.526 15.305c-2.247-.018-3.858-2.23-3.076-4.3a3.31 3.31 0 0 1 2.757-2.11c.384-.04.845-.03 1.215-.098 1.9-.353 3.368-1.806 3.665-3.657.066-.41.031-.9.128-1.335.449-2.016 2.759-3.147 4.699-2.236 1.011.476 1.69 1.374 1.857 2.447q.051.33.034.818c-.22 5.842-5.21 10.519-11.279 10.47m2.831.93a.04.04 0 0 1-.021-.02l-.001-.006.002-.006q0-.003.003-.004l.006-.003q3.458-.792 5.992-3.185.045-.042.083.007c.27.357.554.74.78 1.106a10.6 10.6 0 0 1 1.585 4.89q.037.53.023.819c-.084 1.705-1.51 3.08-3.31 3.09-1.592.01-2.992-1.077-3.294-2.597-.072-.36-.05-.858-.11-1.238q-.282-1.755-1.715-2.84zm-3.369 6.64c-1.353-.235-2.441-1.286-2.684-2.593a5 5 0 0 1-.05-.817V15.14q0-.021.016-.007c.884.786 1.814 1.266 3.028 1.346l.326.01c1.581.051 2.92 1.087 3.229 2.592.457 2.225-1.557 4.195-3.865 3.793"></path>
//             </svg>
//           </div>
//           <div className="flex items-center space-x-1">
//             <Avatar />
//             <p className="flex-1 text-xs text-muted-foreground font-semibold">
//               benevolentnimblebat
//             </p>
//             <ChevronRight className="w-4 h-4" />
//           </div>
//         </div>

//         {/* Desktop Navigation */}
//         <nav className="hidden md:flex items-center gap-3 bg-accent p-1 rounded-2xl shadow">
//           {links.map(({ href, Icon, label }, index) => {
//             const isActive = pathname === href;
//             return (
//               <Link
//                 key={href + index}
//                 href={href}
//                 title={label}
//                 className={cn("p-2 rounded-xl transition-colors", {
//                   "bg-primary text-primary-foreground": isActive,
//                 })}
//               >
//                 <Icon className="h-5 w-5" />
//               </Link>
//             );
//           })}
//         </nav>

//         {/* Right side actions */}
//         <div className="hidden md:flex items-center gap-3">
//           <Link
//             href="#"
//             className="bg-accent rounded-xl p-2 text-sm flex items-center gap-2"
//           >
//             <ImageIcon />
//             Gallery
//           </Link>
//           <Link
//             href="#"
//             className="bg-accent rounded-xl p-2 text-sm flex items-center gap-2"
//           >
//             <Headset className="h-4 w-4" />
//             Support
//           </Link>
//           <Link
//             href="#"
//             className="bg-accent rounded-xl p-2 text-sm flex items-center"
//           >
//             <Bell className="h-4 w-4" />
//           </Link>
//           <Button
//             variant="ghost"
//             size="sm"
//             onClick={() => setTheme(theme === "dark" ? "light" : "dark")}
//             className="h-8 w-8 p-0 bg-accent rounded-xl"
//           >
//             {theme === "dark" ? (
//               <Moon className="h-4 w-4" />
//             ) : (
//               <Sun className="h-4 w-4" />
//             )}
//           </Button>
//           <Link href="#">
//             <Avatar />
//           </Link>
//         </div>
//         {/* Mobile User */}
//         <div className="flex items-center space-x-1 md:hidden">
//           <Button
//             variant="ghost"
//             size="sm"
//             onClick={() => setTheme(theme === "dark" ? "light" : "dark")}
//             className="h-8 w-8 p-0 bg-accent rounded-xl"
//           >
//             {theme === "dark" ? (
//               <Moon className="h-4 w-4" />
//             ) : (
//               <Sun className="h-4 w-4" />
//             )}
//           </Button>
//           <div className="flex items-center space-x-1">
//             <Link href="#">
//               <Avatar />
//             </Link>
//             <p className="flex-1 text-xs text-muted-foreground font-semibold">
//               benevolentnimblebat
//             </p>
//             <ChevronRight className="w-4 h-4" />
//           </div>
//         </div>
//       </div>
//     </header>
//   );
// }

"use client";

import { Button } from "@/components/ui/button";
import {
  Asset,
  Avatar,
  Edit,
  Enhancer,
  Home,
  ImageIcon,
  Menu,
  Realtime,
  Video,
} from "@/hooks/use-icon";
import { cn } from "@/lib/utils";
import { Moon, Sun, ChevronRight, Bell, Headset } from "lucide-react";
import { useTheme } from "next-themes";
import Link from "next/link";
import { usePathname } from "next/navigation";
import { Sheet, SheetContent, SheetTitle, SheetTrigger } from "./ui/sheet";

export function Header() {
  const { theme, setTheme } = useTheme();
  const pathname = usePathname();

  const links = [
    { href: "/", label: "Home", Icon: Home },
    { href: "#", label: "Image", Icon: ImageIcon },
    { href: "#", label: "Video", Icon: Video },
    { href: "#", label: "Enhancer", Icon: Enhancer },
    { href: "#", label: "Realtime", Icon: Realtime },
    { href: "#", label: "Edit", Icon: Edit },
    { href: "#", label: "Assets", Icon: Asset },
    { href: "#", label: "Gallery", Icon: ImageIcon },
    { href: "#", label: "Support", Icon: Headset },
    { href: "#", label: "Notifications", Icon: Bell },
  ];

  return (
    <header className="sticky top-0 z-50 w-full bg-background/80 backdrop-blur-sm">
      <div className="flex h-16 items-center justify-between px-4">
        {/* Mobile nav trigger */}
        <Sheet>
          <SheetTrigger asChild>
            <Button
              variant="ghost"
              size="sm"
              className="h-10 w-10 p-0 lg:hidden"
            >
              <Menu />
            </Button>
          </SheetTrigger>
          <SheetContent side="left" className="w-64">
            <SheetTitle className="sr-only">Krea</SheetTitle>
            <nav className="flex flex-col gap-4 mt-6">
              {links.map(({ href, Icon, label }, index) => (
                <Link
                  key={href + index}
                  href={href}
                  className="flex items-center gap-3 p-2 rounded-md hover:bg-accent"
                >
                  <Icon className="h-5 w-5" />
                  <span>{label}</span>
                </Link>
              ))}
            </nav>
          </SheetContent>
        </Sheet>

        {/* Desktop Logo */}
        <div className="hidden lg:flex items-center gap-4">
          <div>
            <svg
              aria-label="Krea Logo"
              width="24"
              height="24"
              viewBox="0 0 24 24"
              fill="currentColor"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path d="M8.34 1.266c1.766-.124 3.324 1.105 3.551 2.802.216 1.612-.887 3.171-2.545 3.536-.415.092-.877.066-1.317.122a4.63 4.63 0 0 0-2.748 1.34l-.008.004-.01-.001-.006-.005-.003-.009q0-.009.005-.016a.04.04 0 0 0 .007-.022 438 438 0 0 1-.01-4.541c.003-1.68 1.33-3.086 3.085-3.21"></path>
              <path d="M8.526 15.305c-2.247-.018-3.858-2.23-3.076-4.3a3.31 3.31 0 0 1 2.757-2.11c.384-.04.845-.03 1.215-.098 1.9-.353 3.368-1.806 3.665-3.657.066-.41.031-.9.128-1.335.449-2.016 2.759-3.147 4.699-2.236 1.011.476 1.69 1.374 1.857 2.447q.051.33.034.818c-.22 5.842-5.21 10.519-11.279 10.47m2.831.93a.04.04 0 0 1-.021-.02l-.001-.006.002-.006q0-.003.003-.004l.006-.003q3.458-.792 5.992-3.185.045-.042.083.007c.27.357.554.74.78 1.106a10.6 10.6 0 0 1 1.585 4.89q.037.53.023.819c-.084 1.705-1.51 3.08-3.31 3.09-1.592.01-2.992-1.077-3.294-2.597-.072-.36-.05-.858-.11-1.238q-.282-1.755-1.715-2.84zm-3.369 6.64c-1.353-.235-2.441-1.286-2.684-2.593a5 5 0 0 1-.05-.817V15.14q0-.021.016-.007c.884.786 1.814 1.266 3.028 1.346l.326.01c1.581.051 2.92 1.087 3.229 2.592.457 2.225-1.557 4.195-3.865 3.793"></path>
            </svg>
          </div>
          <div className="flex items-center space-x-1">
            <Avatar />
            <p className="flex-1 text-xs text-muted-foreground font-semibold">
              benevolentnimblebat
            </p>
            <ChevronRight className="w-4 h-4" />
          </div>
        </div>

        {/* Desktop Navigation */}
        <nav className="hidden lg:flex items-center gap-3 bg-accent p-1 rounded-2xl shadow">
          {links.slice(0, 7).map(({ href, Icon, label }, index) => {
            const isActive = pathname === href;
            return (
              <Link
                key={href + index}
                href={href}
                title={label}
                className={cn("p-2 rounded-xl transition-colors", {
                  "bg-primary text-primary-foreground": isActive,
                })}
              >
                <Icon className="h-5 w-5" />
              </Link>
            );
          })}
        </nav>

        {/* Right side actions (desktop only) */}
        <div className="hidden lg:flex items-center gap-3">
          {links.slice(7).map(({ href, Icon, label }, index) => (
            <Link
              key={href + index}
              href={href}
              className="bg-accent rounded-xl p-2 text-sm flex items-center gap-2"
            >
              <Icon className="h-4 w-4" />
              {label !== "Notifications" && <span>{label}</span>}
            </Link>
          ))}

          {/* Theme toggle */}
          <Button
            variant="ghost"
            size="sm"
            onClick={() => setTheme(theme === "dark" ? "light" : "dark")}
            className="h-8 w-8 p-0 bg-accent rounded-xl"
          >
            {theme === "dark" ? (
              <Moon className="h-4 w-4" />
            ) : (
              <Sun className="h-4 w-4" />
            )}
          </Button>
          <Link href="#">
            <Avatar />
          </Link>
        </div>

        {/* Mobile User */}
        <div className="flex items-center space-x-2 lg:hidden">
          <Button
            variant="ghost"
            size="sm"
            onClick={() => setTheme(theme === "dark" ? "light" : "dark")}
            className="h-8 w-8 p-0 bg-accent rounded-xl"
          >
            {theme === "dark" ? (
              <Moon className="h-4 w-4" />
            ) : (
              <Sun className="h-4 w-4" />
            )}
          </Button>

          <Link
            href="#"
            className="w-7 h-7 flex items-center rounded-full overflow-hidden"
          >
            <Avatar />
          </Link>
        </div>
      </div>
    </header>
  );
}
